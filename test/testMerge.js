...
function getMergeCalling(nftFrom, nftTo, veARCAddress) {
    const veARC = new web3.eth.Contract(abi, veARCAddress);
    return veARC.methods.merge(nftFrom, nftTo).encodeABI();
}
...
describe("test increase merge", function () {
    ...
    it("at 20 WEEK, create a new lock, merge 7 to 2", async function () {
        ...
        const callings = [
            ...
            getMergeCalling(7, 2, veARC.address)
        ];
    ...
    it("at 20 WEEK, create a new lock, merge 4 to 1", async function () {
        ...
        const callings = [
            getNewLockCalling(locks[8].amount, locks[8].endTime),
            getMergeCalling(4, 1, veARC.address)
        ];
        balance = stringMinus(balance, locks[8].amount);
        await veARC.connect(tester).multicall(callings);
    ...
    it("at 20 WEEK, create a new lock, merge 7 to 6", async function () {
        ...
        const callings = [
            getNewLockCalling(locks[8].amount, locks[8].endTime),
            getMergeCalling(fromId, toId, veARC.address)
        ];
        balance = stringMinus(balance, locks[8].amount);
        await veARC.connect(tester).multicall(callings);
    ...
    it("at 18.9 WEEK, merge 8 to 6", async function () {
        ...
        await veARC.connect(tester).merge(fromId, toId);
