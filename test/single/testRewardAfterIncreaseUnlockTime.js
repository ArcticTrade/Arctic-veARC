...
async function tryIncreaseUnlockTime(veARC, ARC, nftId, tester, endTime) {
    const ARCBalanceBefore = (await ARC.balanceOf(tester.address)).toString();
    await veARC.connect(tester).increaseUnlockTime(nftId, endTime);
    const ARCBalanceAfter = (await ARC.balanceOf(tester.address)).toString();
    return stringMinus(ARCBalanceAfter, ARCBalanceBefore);
}
...
describe("test increase unlock time", function () {
    ...
    it("increase amount", async function () {
        ...
        const reward3 = await tryIncreaseUnlockTime(veARC, ARC, '1', tester, newEndTime);
        ...
        const expectReward3 = stringDiv(stringMul(deltaGlobalAcc3, lastVeARC2), q128);
        expect(reward3).to.equal(expectReward3);

        const stakingStatus3 = await getStakingStatus(veARC, '1');
        const lastVeARC3 = stringMul(slope, remainTime3);
        expect(lastVeARC3).to.equal(stakingStatus3.lastVeARC);
        ...
        const deltaGlobalAcc4 = stringDiv(stringMul(stringMul(rewardPerBlock, '20'), q128), stakeARCAmount);

        const expectReward4 = stringDiv(stringMul(deltaGlobalAcc4, lastVeARC3), q128);
